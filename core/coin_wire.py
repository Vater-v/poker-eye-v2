"""Verified CoinPoker SmartFox (SFS2X) wire codec and EYE->Coin CC resolver.

Ported from the legacy ``ready_v6/coin_action_wire.py`` (observed capture format).
Stdlib-only. This module is pure encoding/decoding and policy projection; it never
performs I/O, scheduling, or state mutation.
"""
...
# SmartFox SFS2X object types used by CoinPoker's ExtensionRequest.
SFS_BYTE = 2
SFS_SHORT = 3
SFS_INT = 4
SFS_UTF = 8
...
# Coin game.user_action codes (verified from captures and legacy resolution).
ACTION_FOLD = 7
ACTION_CHECK = 3
ACTION_CALL = 4
ACTION_RAISE = 5
...
def _enc_value(v: Any) -> bytes:
    if isinstance(v, _Byte):
        return bytes([SFS_BYTE, v.value & 0xFF])
    if isinstance(v, _Short):
        return bytes([SFS_SHORT]) + _i16(v.value)
    if isinstance(v, _Int):
        return bytes([SFS_INT]) + _i32(v.value)
    if isinstance(v, _Str):
        b = v.value.encode("utf-8")
        return bytes([SFS_UTF]) + _u16(len(b)) + b
    if isinstance(v, _Obj):
        out = bytearray([SFS_OBJECT])
        out += _u16(len(v.value))
        for k, x in v.value.items():
            kb = str(k).encode("utf-8")
            out += _u16(len(kb)) + kb + _enc_value(x)
        return bytes(out)
...
def encode_packet(value: dict) -> bytes:
    ...
    if len(payload) > 0xFFFF:
        raise ValueError("SFS packet too large")
...
def build_game_user_action_packet(room_id: int, user_action: int, bet_amount: float = 0.0) -> bytes:
    data = json.dumps(
        {"userAction": int(user_action), "betAmount": float(bet_amount)},
        separators=(",", ":"),
    )
...
def _dec_value(buf: bytes, pos: int) -> Tuple[Any, int]:
    ...
    if typ == SFS_OBJECT:
        ...
        for _ in range(count):
            ln = int.from_bytes(buf[pos:pos + 2], "big")
            pos += 2
            k = buf[pos:pos + ln].decode("utf-8")
            pos += ln
            out[k], pos = _dec_value(buf, pos)
        return out, pos
    raise ValueError(f"unsupported SFS type {typ}")


def decode_packet(raw: bytes) -> dict:
    """Decode a Coin SFS2X frame into a nested dict (best-effort, no payload logs)."""
    if len(raw) < 3:
        raise ValueError("short packet")
    header = raw[0]
    wide = bool(header & 0x08)
    compressed = bool(header & 0x20)
    if wide:
    ...
    obj, _used = _dec_value(payload, 0)
    return obj


def frame_metadata(raw: bytes) -> Dict[str, Any]:
    """Return safe envelope metadata for a raw Coin frame (never the body)."""
    info: Dict[str, Any] = {"length": len(raw)}
    if len(raw) >= 1:
        info["header"] = raw[0]
        info["wide"] = bool(raw[0] & 0x08)
        info["compressed"] = bool(raw[0] & 0x20)
    return info
...
def resolve_eye_cc_action(
    cc: Dict[str, Any],
    *,
    user_turn_options: Optional[dict] = None,
    current_street_bet: float = 0.0,
    chip_scale: int = 100,
) -> ResolvedAction:
    """Project an EYE CC decision onto Coin's paid legal actions.

    Code mapping (verified): FOLD=7, CHECK=3, CALL=4, RAISE=5.
    """
    typ = str(cc.get("type") or cc.get("action") or cc.get("message") or "").strip().upper()
    opts = user_turn_options or {}
    if typ in ("FOLD", "FAST_FOLD"):
        return ResolvedAction("FOLD", ACTION_FOLD, 0.0)
    if typ == "CHECK":
        return ResolvedAction("CHECK", ACTION_CHECK, 0.0)
    if typ == "CALL":
        return ResolvedAction("CALL", ACTION_CALL, 0.0)
    try:
        subtype = int(cc.get("subtype") or 0)
    except (TypeError, ValueError):
        subtype = 0
    message = str(cc.get("message") or "").strip().upper().replace("-", "").replace("_", "").replace(" ", "")
    explicit_allin = typ in ("ALLIN", "ALL-IN", "ALL_IN") or (
        typ in ("RAISE", "BET") and (subtype == 1 or message == "ALLIN")
    )
    if typ in ("RAISE", "BET", "ALLIN", "ALL-IN", "ALL_IN"):
        try:
            amount = float(cc.get("amount"))
        except (TypeError, ValueError):
            amount = None
        desired = (current_street_bet + amount / chip_scale) if amount is not None else None
        candidates = []
        call_opt = _opts_get(opts, "4")
        if isinstance(call_opt, (list, tuple)) and call_opt:
            try:
                ...
                candidates.append((distance, 1, ResolvedAction("CALL", ACTION_CALL, 0.0)))
            except (TypeError, ValueError):
                pass
        rr = _opts_get(opts, "5")
        if isinstance(rr, (list, tuple)) and len(rr) >= 2:
            try:
                ...
                if hi >= lo:
                    ...
                    candidates.append((distance, 0, ResolvedAction("RAISE", ACTION_RAISE, round(target, 8))))
            except (TypeError, ValueError):
                pass
        if candidates:
