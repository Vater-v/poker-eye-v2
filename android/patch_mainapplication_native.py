...
REPLACEMENT = r""".method public onCreate()V
    ...
    invoke-static {p0}, Lcom/hmuriy/HmuriyBridge;->bootstrap(Landroid/content/Context;)V
    :try_end_hmuriy
...
def main():
    ...
    if "HmuriyBridge;->bootstrap(Landroid/content/Context;)V" in source:
        print("[+] MainApplication Hmuriy bootstrap(Context) already present")
        return
    source = replace_method(source)
    if source.count("HmuriyBridge;->bootstrap(Landroid/content/Context;)V") != 1:
        raise RuntimeError("expected exactly one Hmuriy bootstrap(Context) call")
    path.write_text(source, encoding="utf-8", newline="\n")
    print("[+] MainApplication patched: eager Hmuriy bootstrap(Context) with Throwable fail-open")
